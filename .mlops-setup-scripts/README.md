...
* Run `aws configure --profile databricks-mlops-stack` to configure an AWS CLI profile, passing the access key ID and secret access key from the previous step
* Run `export AWS_PROFILE=databricks-mlops-stack` to indicate to Terraform that it should use the newly-created AWS CLI profile
  to authenticate to AWS

### Configure Databricks auth
* Configure a Databricks CLI profile for your staging workspace by running
  ``databricks configure --token --profile "databricks-mlops-stack-staging" --host https://your-staging-workspace.cloud.databricks.com``, 
...
* Configure a Databricks CLI for your prod workspace by running ``databricks configure --token --profile "databricks-mlops-stack-prod" --host https://your-prod-workspace.cloud.databricks.com``
...
Ensure a group named `databricks-mlops-stack-service-principals` exists in the staging and prod workspace, e.g.
...
Then, grant the `databricks-mlops-stack-service-principals` group [token usage permissions](https://docs.databricks.com/administration-guide/access-control/tokens.html#manage-token-permissions-using-the-admin-console)
