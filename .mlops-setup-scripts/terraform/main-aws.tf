...
resource "aws_s3_bucket" "terraform_state" {
  bucket = "databricks-mlops-stack-tfstate"
...
resource "aws_dynamodb_table" "terraform_state_lock" {
  name           = "databricks-mlops-stack-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
...
resource "aws_iam_policy" "my-s3-read-policy" {
  name        = "databricks-mlops-stack-terraform-state-manage"
  description = "S3 bucket access policy for managing terraform state for databricks-mlops-stack"
...
resource "aws_iam_user" "s3_bucket_cicd_user" {
  name = "databricks-mlops-stack-terraform-cicd-user"
}
...
resource "aws_s3_bucket" "cicd_terraform_state" {
  bucket = "databricks-mlops-stack-cicd-setup-tfstate"
...
resource "aws_dynamodb_table" "cicd_terraform_state_lock" {
  name           = "databricks-mlops-stack-cicd-setup-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
